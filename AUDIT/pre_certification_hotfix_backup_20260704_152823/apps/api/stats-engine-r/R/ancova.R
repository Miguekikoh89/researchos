...
run_ancova <- function(df,dep_items,group_var,covariate_items,dep_name,alpha=.05,posthoc="bonferroni",check_slopes="yes"){
  tryCatch({
    ...
    list(n=nrow(datos),dep_var=dep_name,group_var=group_var,ancova_table=rows,adjusted_means=adj,posthoc_adjusted_means=pairs_out,posthoc_method=am,homogeneity_slopes=slopes,
      ...
      partial_eta2_group=as.numeric(pes_group),partial_eta2_covariate=as.numeric(pes_cov),anova_type="Secuencial (Type I): covariable antes de grupo",significant=isTRUE(gp<alpha),p_grupo=as.numeric(gp),decision=if(gp<alpha)paste0("Existen diferencias significativas en ",dep_name," entre grupos, controlando la covariable")else"No existen diferencias significativas entre grupos al controlar la covariable")
