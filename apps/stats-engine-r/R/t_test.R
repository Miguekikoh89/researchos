...
levene_test <- function(x1,x2) {
  tryCatch({
    ...
    list(F=round(F_val,3),df1=1,df2=n1+n2-2,p=round(p_val,4),equal_variances=p_val>=0.05)
...
normality_by_group <- function(x1,x2,alpha=0.05) {
  ...
  list(group1=list(W=round(t1$statistic,4),p=round(t1$p.value,4),normal=t1$p.value>=alpha),
       group2=list(W=round(t2$statistic,4),p=round(t2$p.value,4),normal=t2$p.value>=alpha),
...
t_independent <- function(x1,x2,alpha=0.05,group_names=c("Grupo 1","Grupo 2")) {
  x1<-x1[!is.na(x1)]; x2<-x2[!is.na(x2)]; n1<-length(x1); n2<-length(x2)
  lev<-levene_test(x1,x2); norm<-normality_by_group(x1,x2,alpha)
  t_eq<-t.test(x1,x2,var.equal=TRUE,alternative="two.sided")
  t_we<-t.test(x1,x2,var.equal=FALSE,alternative="two.sided")
  ...
  list(test_type="t_independiente",method_used=if(lev$equal_variances)"Student (varianzas iguales)" else "Welch (varianzas desiguales)",
    t=round(t_use$statistic,4),df=round(t_use$parameter,2),p=round(t_use$p.value,4),
    ...
    t_student=round(t_eq$statistic,4),df_student=t_eq$parameter,p_student=round(t_eq$p.value,4),
    t_welch=round(t_we$statistic,4),df_welch=round(t_we$parameter,2),p_welch=round(t_we$p.value,4),
    ...
    significant=sig,decision=if(sig)"Se rechaza H0" else "No se rechaza H0",alpha=alpha)
}
t_paired <- function(x1,x2,alpha=0.05,group_names=c("Pre","Post")) {
  ...
  t_res<-t.test(x1,x2,paired=TRUE,alternative="two.sided"); d<-cohen_d(x1,x2,paired=TRUE); sig<-t_res$p.value<alpha
  list(test_type="t_pareada",t=round(t_res$statistic,4),df=t_res$parameter,p=round(t_res$p.value,4),
    ...
    normality_diff=if(!is.null(sw))list(W=round(sw$statistic,4),p=round(sw$p.value,4),normal=sw$p.value>=alpha) else list(W=NA,p=NA,normal=NA),
...
mann_whitney <- function(x1,x2,alpha=0.05,group_names=c("Grupo 1","Grupo 2")) {
  x1<-x1[!is.na(x1)]; x2<-x2[!is.na(x2)]; n1<-length(x1); n2<-length(x2)
  test<-wilcox.test(x1,x2,alternative="two.sided",correct=TRUE,conf.int=TRUE)
  U<-test$statistic; r_rb<-1-(2*U)/(n1*n2); sig<-test$p.value<alpha
  list(test_type="mann_whitney",U=as.numeric(U),p=round(test$p.value,4),
...
wilcoxon_paired <- function(x1,x2,alpha=0.05,group_names=c("Pre","Post")) {
  valid<-complete.cases(x1,x2); x1<-x1[valid]; x2<-x2[valid]; n<-length(x1)
  test<-wilcox.test(x1,x2,paired=TRUE,alternative="two.sided",correct=TRUE)
  W<-test$statistic; r_rb<-W/(n*(n+1)/2); sig<-test$p.value<alpha
  list(test_type="wilcoxon_pareado",W=as.numeric(W),p=round(test$p.value,4),
    p_apa=if(test$p.value<.001)"< .001" else paste0("= ",formatC(test$p.value,digits=3,format="f")),
    r_rb=round(r_rb,3),
...
compute_ttest <- function(x1,x2,type="independiente",alpha=0.05,group_names=c("Grupo 1","Grupo 2"),force_nonparametric=FALSE) {
  ...
  if(type=="pareada") {
    ...
    if(force_nonparametric||sw$p.value<alpha) { res<-wilcoxon_paired(x1,x2,alpha,group_names); res$auto_selected<-"Wilcoxon (diferencias no normales)" }
    else { res<-t_paired(x1,x2,alpha,group_names); res$auto_selected<-"t pareada (diferencias normales)" }
  } else {
    norm<-normality_by_group(x1,x2,alpha)
    if(force_nonparametric||!norm$both_normal) { res<-mann_whitney(x1,x2,alpha,group_names); res$auto_selected<-"Mann-Whitney (distribucion no normal)" }
    else { res<-t_independent(x1,x2,alpha,group_names); res$auto_selected<-res$method_used }
  }
  res
}
